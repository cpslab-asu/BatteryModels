%% UAV Simulation
function [t,y] = IntegrationPsiTaliro(tI,tP,gA)
    takenOff = 0;
    startIndex = 0;
    % Define parameters
    ...
    for i = 2:size(pulse_signal,2)
        ...
        %if(i > 300 && i < 500)
        if(i < 0)
            adjY =  awgn(Y(3,i-1),SNR,SPower,'linear');
        else
            adjY = Y(3,i-1);
        end
        err(i-1) = SP - adjY;
        ...
        Y(:,i) = Y(:,i-1) + (1/fs)*stateDiff(Y(:,i-1),U(:,i-1),i-1,tI,tP, gA);
        if(Y(3,i) > 0)
            takenOff = 1;
        end
...
function y = stateDiff(X,U,i, tI, tP, gA)
    m = 0.65; % kg
    g = 9.806; % m/s^2
    l = 0.232; % m
    rho = 1.293; % kg/m^3
